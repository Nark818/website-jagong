...
import { CAT_LABEL, CAT_BADGE_CLASSES, CAT_ICON, TAG_CLASSES } from "@/lib/news";
import { formatIndonesianDate } from "@/lib/format";
import { getNewsPosts, getNewsPostBySlug } from "@/lib/supabase/queries";
import { createStaticClient } from "@/lib/supabase/static";

export async function generateStaticParams() {
  // Build-time context has no HTTP request, so this can't use the
  // cookie-aware server client (getNewsPosts) — use the static client instead.
...
export default async function BeritaDetailPage({
  ...
  const CatIcon = CAT_ICON[news.category];
  const sameCategory = allPosts.filter(
    (item) => item.slug !== news.slug && item.category === news.category,
  );
  const related = (
    sameCategory.length > 0
      ? sameCategory
      : allPosts.filter((item) => item.slug !== news.slug)
  ).slice(0, 3);
      ...
      <section className="bg-ocean-800 px-6 pt-10 pb-20">
        <div className="mx-auto max-w-[820px]">
          ...
          <div className="mt-4 inline-flex items-center gap-1.5 text-sm text-ocean-100">
            ...
            {formatIndonesianDate(news.published_at)}
          </div>
        </div>
      </section>
