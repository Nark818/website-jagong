...
const GENDER_DATA = [
  { label: "Laki-laki", value: 2098, color: "#0891B2" },
  { label: "Perempuan", value: 2146, color: "#1F8347" },
];

const RW_CHART_COLORS = ["#0891B2", "#1F8347", "#CA8A04"];
const RW_DATA = RW_ROWS.map((r, i) => ({
  label: r.name,
  value: r.rumah,
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <div className="overflow-hidden rounded-lg border border-border-default bg-surface-card">
            <div className="border-b border-border-default bg-surface-sunken px-5 py-3.5">
              ...
              <div className="mt-0.5 text-[12px] text-text-muted">
                Pokok: {fmt(pokok.stts)} STTS · Rp {fmt(pokok.rp)}
              </div>
            </div>
            ...
            <TaxRow label="S/D Bulan Lalu" stts={fmt(pbbLalu.stts)} rp={fmt(pbbLalu.rp)} />
            <TaxRow label="Bulan Ini" stts={fmt(pbbIni.stts)} rp={fmt(pbbIni.rp)} />
            <TaxRow
              label="S/D Bulan Ini"
          ...
          <div className="overflow-hidden rounded-lg border border-border-default bg-surface-card">
            <div className="border-b border-border-default bg-surface-sunken px-5 py-3.5">
              ...
              <div className="mt-0.5 text-[12px] text-text-muted">
                Jumlah tunggakan: {fmt(tunggakan.stts)} STTS · Rp{" "}
                {fmt(tunggakan.rp)}
              </div>
            </div>
            <div className="grid grid-cols-[1.4fr_0.8fr_1.1fr] border-b border-border-default">
            ...
            <TaxRow label="S/D Bulan Lalu" stts={fmt(tdLalu.stts)} rp={fmt(tdLalu.rp)} />
            <TaxRow label="Bulan Ini" stts={fmt(tdIni.stts)} rp={fmt(tdIni.rp)} />
            <TaxRow
              label="S/D Bulan Ini"
